...
function subscribe() {
    ...
    ws.on('error', function() {
        console.log('sol socket error')
    })

    ws.on('close', function () {
        console.log('sol socket close');
        setTimeout(subscribe, 1000);
    })
}

async function getAddresses() {
    let res = await axios.post(config.get('BLOCKER_URL') + '/addresses/sol', { 'secret': config.get('BLOCKER_SECRET') });
...
function broadcast(block, afterSeconds = 10) {
    setTimeout(() => {
        try {
            axios.post(config.get('BLOCKER_URL') + '/transactions/solana', { block, type: 'standard' })
    
            console.log('Succeed broadcast sol block ' + block)
        } catch (e) {
            console.error(e);
        }
    }, afterSeconds * 1000)
